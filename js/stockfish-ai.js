...
async function initializeStockfishEngine() {    
    try {    
        // MODIFICACIÓN CLAVE AQUÍ: Esperar a que window.Stockfish esté definido    
        console.log('⏳ Esperando a que Stockfish esté disponible...');  
        await new Promise(resolve => {  
            const checkStockfish = () => {  
                if (window.Stockfish) {  
                    resolve();  
                } else {  
                    setTimeout(checkStockfish, 100); // Reintentar cada 100ms  
                }  
            };  
            checkStockfish();  
        });  
        ...
        engine = window.Stockfish; // Ahora Stockfish debería estar definido    
        ...
        engineReady = false;    
        window.engineReady = false; // HACER ESTA VARIABLE GLOBALMENTE ACCESSIBLE    
